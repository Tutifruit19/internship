...
def sum_jurek_tab(list_years,pol):
    ...
    for p in range(len(list_years)):
        if p==2015:
            ...
            path_jurek_helcom = "data/data_jurek_helcom/"+list_years[p]+pol+"_"+"LF.csv"
            path_jurek_ospar = "data/data_jurek_ospar/"+list_years[p]+pol+"_"+"LF.csv"
...
def scaling(list_years,pol_dep,pol_sr):
    if pol_sr == "reduced_nitrogen":
        SR_1 = sum_jurek_tab(list_years,"reduced_nitrogen")
        SR_2 = sum_jurek_tab(list_years,"reduced_nitrogen")
    ...
    if type_pol == "oxidised_nitrogen" or type_pol == "dry_oxidised_nitrogen" or type_pol == "wet_oxidised_nitrogen":
        ...
        for i in range(len(list_years)):
            if int(i) == 2015:  #no tab for 2015 so we need to ignore the year in the loop
                ...
                if type_pol == "reduced_nitrogen_2":#we need sum wet + dry. Open all the tab that we need
                    ...
                    path_heiko = "data/data_emi_normalization/"+list_years[i]+"_reduced_nitrogenLF.csv"
                    path_jurek_helcom = "data/data_jurek_helcom/"+list_years[i]+"_reduced_nitrogenLF.csv"
                    path_jurek_ospar = "data/data_jurek_ospar/"+list_years[i]+"_reduced_nitrogenLF.csv"
                    tc.append(unit_normalization(path_heiko,path_jurek_helcom,path_jurek_ospar)[1])
                    tc_sources.append(unit_normalization(path_heiko,path_jurek_helcom,path_jurek_ospar)[0])
                    receptors = unit_normalization(path_heiko,path_jurek_helcom,path_jurek_ospar)[2]
        if type_pol == "reduced_nitrogen_2":#if type_pol = oxidised_nitrogen it does the sum of dry + wet
            for k in range(len(tc)):
                for i in range(np.shape(tc[k])[0]):
                    for j in range(np.shape(tc[k])[1]):
                        tc[k][i][j] = tc[k][i][j] + tc_bis[k][i][j]
